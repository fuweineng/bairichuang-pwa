...
async function init() {
  ...
  state.settings = await get(K.SETTINGS) || { section: 'junior', sectionVersions: {}, weakThreshold: 0.6, lastQuestionBankUpdate: null, appVersion: '9.8.6', audioVersion: '', questionBankVersion: '', soundEnabled: true };
...
function renderQuestion() {
  ...
  const IMG_CDN = 'https://cdn.jsdelivr.net/gh/fuweineng/bairichuang-pwa@main/images/';
  const imageSources = Array.isArray(q.images)
    ? q.images
    : [q.imageUrl || q.image].filter(Boolean);
  const imageBlock = imageSources.length > 0
    ? `<div class="question-image-grid">${imageSources.map(src => `<img class="question-image" src="${IMG_CDN}${src}" alt="题目配图" loading="lazy" />`).join('')}</div>`
    : '';
...
async function clearAllData() {
  ...
  state.sessions = [];
  state.meta = {};
  state.settings = { weakThreshold: 0.6, lastQuestionBankUpdate: null, appVersion: '9.8.6', audioVersion: '', questionBankVersion: '', soundEnabled: true };
